...
HRESULT CSingleTexture::Insert_Texture(const wstring& pFilePath, const wstring& pStateKey, const int iCnt)
{
    m_pTexInfo = new TEXINFO;
    ZeroMemory(m_pTexInfo, sizeof(TEXINFO));

    // D3DXGetImageInfoFromFile : 지정한 이미지 파일에 관한 정보를 얻어와 구조체에 기록하는 함수
    if (FAILED(D3DXGetImageInfoFromFile(pFilePath.c_str(), &(m_pTexInfo->tImgInfo))))
    {
